...
impl<'a, G: BorrowMut<MlsGroup>> CommitBuilder<'a, LoadedPsks, G> {
    ...
    fn build_internal<S: Signer>(
        ...
        > {
            ...
            let mut _update_count = 0usize;

            for queued in proposal_queue.queued_proposals() {
                match queued.proposal() {
                    Proposal::Add(_) => add_count += 1,
                    Proposal::Remove(_) => remove_count += 1,
                    Proposal::Update(_) => _update_count += 1,
                    _ => {}
                }
            }
            ...
            let (welcome_option, group_info) = if !needs_group_info {
                ...
                let welcome_option = needs_welcome
                    .then(|| -> Result<_, CreateCommitError> {
                        ...
                        #[cfg(feature = "profiling-json")]
                        let tree_size = diff.tree_size().u32();
            ...
            let final_group_epoch = diff.group_context().epoch().as_u64();
            let final_tree_size = diff.tree_size().u32();
            let final_ciphersuite = format!("{:?}", ciphersuite);
        ...
        #[cfg(feature = "profiling-json")]
        {
            ...
            let allocation_info = measure(|| {
                measured_result = Some(build_commit());
            });
            ...
            if let Some(scope) = commit_scope {
                let mut event = scope.finish();
                event.op = final_commit_op.to_string();
                event.group_epoch = Some(final_group_epoch);
