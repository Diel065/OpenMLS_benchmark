...
RUN mkdir -p /out \
    && cp /app/target/x86_64-unknown-linux-musl/release/key_repository /out/key_repository \
...
# ---------- kr runtime ----------
FROM scratch AS kr-runtime
COPY --from=builder /out/key_repository /key_repository
ENTRYPOINT ["/key_repository"]
