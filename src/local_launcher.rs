...
impl Drop for LocalDeployment {
    fn drop(&mut self) {
        if let Some(child) = self.kr_child.as_mut() {
            let _ = child.kill();
            let _ = child.wait();
        }
...
pub fn launch_local_stack(config: &LocalLaunchConfig) -> Result<LocalDeployment> {
    ...
    let (kr_bin, relay_bin, worker_bin) = ensure_binaries_available(&bin_dir)?;

    let kr_url = format!("http://{}", config.kr_listen_addr);

    let relay_listen_addr = derive_relay_listen_addr(config.kr_listen_addr)?;
    let relay_url = format!("http://{}", relay_listen_addr);

    let kr_log_path = run_dir.join("kr.log");
    let kr_log = File::create(&kr_log_path)
        .with_context(|| format!("Could not create {}", kr_log_path.display()))?;
    let kr_log_err = kr_log.try_clone()?;

    let kr_child = Command::new(&kr_bin)
        .arg("--listen-addr")
        .arg(config.kr_listen_addr.to_string())
        .stdout(Stdio::from(kr_log))
        .stderr(Stdio::from(kr_log_err))
        .spawn()
        .with_context(|| format!("Could not spawn KR binary at {}", kr_bin.display()))?;
    ...
    for i in 0..config.worker_count {
        ...
        let profile_path = run_dir.join(format!("participant-{}.jsonl", id));
        ...
        workers.push(WorkerSpec::legacy(id, worker_url));
    ...
    Ok(LocalDeployment {
        ...
        relay_url,
        workers,
        kr_child: Some(kr_child),
        relay_child: Some(relay_child),
        worker_children,
    })
}

fn derive_relay_listen_addr(kr_listen_addr: SocketAddr) -> Result<SocketAddr> {
    let relay_port = kr_listen_addr
        .port()
        .checked_add(1000)
        .ok_or_else(|| anyhow!("Relay port overflow from KR port {}", kr_listen_addr.port()))?;

    Ok(SocketAddr::new(
        ip_for_relay(kr_listen_addr.ip()),
        relay_port,
    ))
}
